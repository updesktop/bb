...
function jeff(){ 
  alert(          
    ...
    '\nDB_AREA: '+DB_AREA.length+
    '\nDB_PTR: '+DB_PTR.length+
    '\nDB_PTR2: '+DB_PTR2.length+
    '\nDB_RET: '+DB_RET.length+
    '\nDB_RET2: '+DB_RET2.length
  );
}
...
async function xget_db_all(tbl){   
...
async function get_db_all(tbl){   
  ...
  //if(tbl=='ptr'){ clear_ptr(); }
  //if(tbl=='ptr'){ clear_db(tbl); }
  await axios.get('/items', { params: {tbl:tbl} })
  .then(function (response) {
    //console.log(response.data);
    var aryDB = response.data;
    //alert('DB_ALL len:'+aryDB.length);
