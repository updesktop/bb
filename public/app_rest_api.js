function rest_api_start(){  
  showProgress(true);
  axios.get('/items', { params: {tbl:'user'} } ).then(function (response){ mod1(response); }).catch(function (error) { console.log(error); });
  /*  
  if(DEBUG_NODE){
  ...
  */
  function mod1(response){
    ...
    //alert(DB_USER.length+' clients:'+DB_CLIENTS.length);
...
function rest_api_get_all_tables(){
  axios.get('/items/get_all_tables', { params: {tbl:['user','area','ret','ret2','ptr','ptr2','whouse']} }).then(function (response){ mod2(response); }).catch(function (error) { console.log(error); });
...
function rest_api_lognow(u,p){
  axios.get('/items/get_user', { params: {userid:u,pword:p} } ).then(function (response){ api_lognow(response); }).catch(function (error) { console.log(error); });
  /*
  if(DEBUG_NODE){
